...
| **Home carousel autoplays on phones** — an Android phone reported the band never autoplays; investigated on the #179 preview (bandwidth ruled out on a throttled emulation), the causes were `prefers-reduced-motion` gating the script (Android "Remove animations", iOS Reduce Motion), phones refusing a script-started play() until a touch, and WebKit wanting the `muted` ATTRIBUTE. Operator decisions: the films autoplay under reduced motion too (content with a pause control — the toggle; only the crossfade stands down), a refused play() retries inside the first gesture, the built video carries the attributes. ~0.6KB more script | **Merged into `phase-c`** 2026-09-03 (operator: "Go ahead and merge #180"; after #182, via a merge of phase-c) | DECISIONS 2026-09-03; CLAUDE.md script list; BUILD_SPEC §9 |
| **Home: scale, rhythm, and four new moves** — the 2026-09-03 design critique's Tier 1 + 2 (operator: "boring" → mockup approved → "create what you are proposing"): at ≥900px the hero photo bleeds toward a full-clamp headline over a one-sentence lead (copy on solid noir — the plate rule kept), the carousel band composed (the recorded heading as two display-1 lines + the controls beside the film — WITHDRAWN 2026-09-04 at the operator's direction, "look like what we have in PR #149": the band is phase-c's again, the slide settle with it; the slide order briefly put Amy's films first and was restored to the recorded J1 → studio → J2 → team at the operator's direction the same day), display-size section openers over one-sentence decks (the §6 credentials kept as a compact line), and a new noir statement band ("Amy comes to you." beside the van interior — /mobile's first home-page door; a full-bleed backdrop, the fifth arch exemption) between the doors and the visit beat; the motion vocabulary +4 transform-only moves (settle, drift, stagger, hover-scale; plus the carousel's slide settle), zero JS added; phones unchanged below 900px. Three hero-headline candidates on the PR for Amy. **Concept layer, same day** (the operator lifted the zero-JS rule for this branch as a concept test): self-hosted GSAP 3.15 + Lenis 1.3 — the wordmark's neon switch-on, word-by-word headline and openers, Amy's studio reel (its three treatment passages, looped) in the hero over her portrait, a chevron scroll cue, the scrubbed hero exit, dealt doors, arch reveals, band parallax, a cursor light over noir; off under reduced motion; the home URL's script budget raised 30→80KB on this branch only (a third gate row — the preview workflow gates before it deploys), every other page and budget unchanged | **Merged into `phase-c`** 2026-09-04 (operator: "go ahead and merge #179 and refresh the previews" — with tweaks 4–6: the reel continuous with a portrait beat, the film band back to phase-c's; the branch-scoped lifts became standing decisions: CLAUDE.md's fourth consumer, the home Lighthouse row, the hero-H1 exception; Amy's reaction pending on the standing demo) | DECISIONS 2026-09-03 + addenda and the adoption paragraph; CLAUDE.md locked decisions; BUILD_SPEC §5/§13; global.css motion header; lighthouserc.json; docs/HOME-CONCEPT.md (the working record) |
| **/mobile — the viewfinder film** — the friends section's film re-shot from Amy's own 1080×1920 clip, sent to the operator directly (a camcorder-viewfinder template burned in; the seated guest's release confirmed on file — no preview-only flag): content-named `van-viewfinder-treatment`, the 24rem-slot recipe (810×1440, 5.39MB), muted because the song is a commercial recording (the operator's listen — music only — is on the record; a sounded cut waits on a license), the wrapper's cap lifted from 18rem to the row's 24rem as the 2026-09-02 entry pre-authorized; the van film's poster and captions deleted, its Blob object deleted once unreferenced; the template's 2.33 s tiled reveal trimmed on the preview at the operator's direction (stem `van-viewfinder-treatment-trim`, 10.1 s). The treatment-film phone-autoplay policy deferred to its own PR | **Merged into `phase-c`** 2026-09-03 (operator approval on the PR #181 preview — "we are good on the music and the merge": the muted rendition stands, no license sought; Amy's reaction pending on the standing demo) | DECISIONS 2026-09-03; BUILD_SPEC §6; CLINICIAN-SIGN-OFF non-gated section |
| **Treatment films take the carousel's phone policy** — `treatment-video.js` no longer bails under prefers-reduced-motion (the films are content with a pause control — the native controls; the 2026-09-03 carousel decision extended to all eight opted-in players on six pages at the operator's direction), a refused play() is retried inside the person's first gesture, and the muted/playsinline attributes are set before the first attempt (the WebKit quirk); ~1.3KB more script, budget untouched. The component, spec, runbook, and the six consumers' comments follow — the two treatment MDX comments in their own content commit | **Merged into `phase-c`** 2026-09-03 (operator approval on the PR #182 preview — "go ahead and merge"; the real-phone check stays theirs) | DECISIONS 2026-09-03; CLAUDE.md script list; BUILD_SPEC §9; RUNBOOK |
