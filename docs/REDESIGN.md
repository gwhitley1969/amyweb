...
| **The home hero ARRIVAL** — a linear.app-inspired load entrance on the existing hero (yardstick gap 6, choreographed restrained motion): staggered fade-up of the copy column (eyebrow → H1 → lead → CTAs → chips, 100–580ms delays, 600ms quintic ease-outs), the photo warming from dark by filter only (the LCP element never leaves opacity 1 — perf budgets untouched), the neon bloom igniting last (~1.85s settle). Pure CSS, zero JS, hero only, settled page pixel-identical; ships under operator override of the motion rulebook's no-hero-entrance rule (their own words, scoped to the home hero alone); reduced motion serves the settled page instantly (the arrival carries its own stop — the global kill switch misses delays); /styleguide/concept plays the same arrival | **In progress** (2026-08-28) | DECISIONS 2026-08-28; global.css motion rulebook; BUILD_SPEC §5; CLINICIAN-SIGN-OFF drift list |
